...
def sequence_mask(length, max_length=128):
...
class InitialReconstruction(BaseModule):
    ...
    def forward(self, stftm):
        ...
        istft = torch.istft(stft, n_fft=self.n_fft, hop_length=self.hop_size, win_length=self.n_fft, window=self.window, center=True)
        # torch.istft(stft, n_fft=self.n_fft, hop_length=self.hop_size, win_length=self.n_fft, window=self.window, center=True)
