# Original IDMap paper implementation (legacy)

The original paper's inference sources remain in [`IDMap-MLP/`](../IDMap-MLP/)
and [`IDMap-Diff/`](../IDMap-Diff/). They were written for the original
...
The historical environment uses the top-level `requirements.txt` and the
`SA-toolkit/` tree. The historical entry points are `IDMap-MLP/infer.py` and
`IDMap-Diff/infer.py`; they require their original feature/model assets and
